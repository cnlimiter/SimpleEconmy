package cn.evolvefield.mods.simpleeco.core.CShop;
...
public class CShopLoader {
    ...
    private static void activateShop(TileEntity storage, SignTileEntity tile, World world, BlockPos pos, CompoundNBT nbt, PlayerEntity player) {
        ...
        if (actionEntry.getContents().equalsIgnoreCase("[buy]")
                ...
                || actionEntry.getContents().equalsIgnoreCase("[s-buy]")
                || actionEntry.getContents().equalsIgnoreCase("[s-sell]")) {
            //second confirm the price value is valid
            if (actionEntry.getContents().equalsIgnoreCase("[s-buy]") || actionEntry.getContents().equalsIgnoreCase("[server-sell]")) {
    ...
    private static void getSaleInfo(CompoundNBT nbt, PlayerEntity player) {
        ...
        if (isBuy)
            player.sendMessage(new TranslationTextComponent("message.shop.info.buy", itemComponent, SEConfig.CURRENCY_SYMBOL.get()+ value), player.getUUID());
        else
            player.sendMessage(new TranslationTextComponent("message.shop.info.sell", itemComponent,SEConfig.CURRENCY_SYMBOL.get()+ value ), player.getUUID());
